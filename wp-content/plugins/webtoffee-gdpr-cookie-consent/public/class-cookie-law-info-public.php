		    ...
		    if(is_object($term))
		    {
	            ...
	            */
		    	...
		    	if(function_exists('get_term_meta'))
		    	{
					...
					$term_ccpa_optout = get_term_meta($term->term_id,'CLIccpaoptout',true);
					...
					if( !empty( $term_ccpa_optout ) ) {
						$posts[$term_slug]['ccpa_optout'] = $term_ccpa_optout;
					}
		    	}		    	
			}
		}
	...
	public function include_user_accepted_cookielawinfo()
	{
	     ...
	     if($the_options['is_on'] == true && !is_admin()) 
	     {
	        ...
	        {	  
				...
				if(isset($cookie['ccpa_optout']) && $cookie['ccpa_optout']==1){
					$is_cca_applicable = 'true';  
				}	 	        
				unset($cookie['loadonstart']);
				unset($cookie['ccpa_optout']); 
				...
				{	
					...
					{	
					$wt_cli_replace = 'data-cli-class="cli-blocker-script"  data-cli-category="'.$key.'" data-cli-script-type="'.$key.'" data-cli-block="'.$is_script_block.'" data-cli-block-if-ccpa-optout="'.$is_cca_applicable.'" data-cli-element-position="head" ';
	    ...
	    {	        
	        ...
	        {    
				...
				{     
					...
					{	
						$wt_cli_replace = 'data-cli-class="cli-blocker-script"  data-cli-category="'.$key.'" data-cli-script-type="'.$key.'" data-cli-block="'.$is_script_block.'" data-cli-block-if-ccpa-optout="'.$is_cca_applicable.'" data-cli-element-position="body"';
