...
class Cookie_Law_Info_Script_Blocker_Frontend
{
                ...
                {   
                    ...
                    if(isset($thirdPartyScript[$scriptkey])) {   
                        if($v->cliscript_status=='yes') //assign category slug for admin enabled categories
                        {
                            ...
                            $thirdPartyScript[$scriptkey]['ccpa_optout'] ='false';
                            ...
                            if($v->category_slug!="" || $v->category_slug !== NULL) { //a category assigned
                                ...
                                $term_ccpa_optout = get_term_meta( $category->term_id, 'CLIccpaoptout', true);
                                if( $term_ccpa_optout == 1)
                                {
                                    $thirdPartyScript[$scriptkey]['ccpa_optout'] = 'true';
                                }
                ...
                if(isset($v['category']))
                {
                    ...
                    {   
                        ...
                        {   
                            ...
                            if( isset($thirdPartyScript[$k]['ccpa_optout']) && $thirdPartyScript[$k]['ccpa_optout'] === 'true' && $ccpa_optout === true  ) {
    ...
    {   
        ...
        return preg_replace_callback($pattern,function( $matches ) use ( $autoData, $elm_position ){
            ...
            $ccpa_optout = $autoData['ccpa_optout'];
            ...
            if( strpos($match, 'data-cli-class') === false ){
                ...
                if ( (preg_match('/<iframe.*(src\s*=\s*(?:"|\')(.*)(?:"|\')).*>.*<\/iframe>/i', $match, $element_match)) ||(preg_match('/<object.*(src=\"(.*)\").*>.*<\/object >/', $match, $element_match)) || (preg_match('/<embed.*(src=\"(.*)\").*>/', $match, $element_match)) || (preg_match('/<img.*(src=\"(.*)\").*>/', $match, $element_match)) ) {
                
                    $element_src = $element_match[1];
                    $element_modded_src = preg_replace('/(src=)/',$wt_cli_replace.' data-cli-placeholder="'.$placeholder_text.'" data-cli-src=',$element_src,1);
