 ...
 */
	...
	foreach($extentions as $key => $addon){
		...
		$addon['url'] = add_query_arg( array(  'utm_source' => 'extensions', 'utm_medium' => 'liteplugin', 'utm_campaign' => 'pro_extensions'  ),  esc_url($addon['url']) );

		$html .= '<div class="fc-3">
					<a target="_blank" href="'.esc_url( $addon['url'] ).'"><img src="'.esc_url( plugins_url($form->options['productSlug']).'/assets/images/extentions/'.$addon['thumb'] ).'"/></a>
		   		 </div>';
