...
class Realia_Post_Type_Property {
	...
	public static function get_properties( $user_id = null, $status = 'any', $parent_property_id = null ) {
		...
		//BAPTISTE ADD
		if(!is_null($parent_property_id)){
			$posts = $query->get_posts();
			if ( is_array( $posts ) && ! empty( $posts ) ){
				return $posts;
			}else{
				$parentId = self::get_parent_property_id($parent_property_id);
				
				if($parentId){
					$args['meta_value'] = $parentId;
		...
		return $query->get_posts();//OLD DEFAULT BEHAVIOR
	}
	public static function get_parent_property_id($childId){//BAPTISTE ADD
		global $wpdb;
		$req = $wpdb->get_results( "SELECT meta_value FROM $wpdb->prefix"."postmeta WHERE post_id='$childId' AND meta_key='".REALIA_PROPERTY_PREFIX ."parent_property' LIMIT 1");
				
		if(is_array($req) && !empty($req)){
			return $req[0]->meta_value;
		}
		return false;
	}
