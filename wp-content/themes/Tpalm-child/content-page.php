			...
			<div class="<?php echo esc_attr($thegem_center_classes); ?>">
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<div class="entry-content post-content">
                        <div class="breadcrumbs bc_top" style="margin-top:20px; margin-bottom:20px" typeof="BreadcrumbList" vocab="https://schema.org/">
    <?php if(function_exists('bcn_display'))
    {

		/*
    	$categoryId137 = get_the_category();//BAPTISTE ADD
    	if($categoryId137[0]->cat_ID === 137 && get_post_type() === 'post'){//BAPTISTE ADD
    		add_action('bcn_after_fill', 'baptiste_insertafterfirst_breadcrumb_adder');
				function baptiste_insertafterfirst_breadcrumb_adder($breadcrumb_trail)
				{	
			    $breadcrumb_trail->insertafterfirst(new bcn_breadcrumb(get_the_title(24564), NULL, array(), get_permalink(24564)));
				}
    	}

    	if(get_post_type() === 'thegem_pf_item'){//BAPTISTE ADD
	    	$lastUrl = false;
	    	$lastTitle = false;
	    	$lastName = false;
	    	$thegem_categories = get_the_terms(get_the_ID(), 'thegem_portfolios');
				...
				if($thegem_categories) {
					foreach($thegem_categories as $thegem_category) {
						$lastUrl = esc_url(get_category_link( $thegem_category->term_id ));
						$lastTitle = esc_attr( sprintf( __( "View all posts in %s", "thegem" ), $thegem_category->name ) );
						$lastName = $thegem_category->name;
					}
				}
				if($lastName){
					if(!defined('thegem_pf_item_LASTNAME')){
						define('thegem_pf_item_LASTNAME', $lastName);
						define('thegem_pf_item_LASTURL', $lastUrl);
					}
	*//*
					add_action('bcn_after_fill', 'baptiste_insertbeforelast_breadcrumb_adder');
					function baptiste_insertbeforelast_breadcrumb_adder($breadcrumb_trail)
					{
					    $breadcrumb_trail->insertbeforelast(new bcn_breadcrumb(thegem_pf_item_LASTNAME, NULL, array(), thegem_pf_item_LASTURL));
					}
	*//*
				}
    	}*/
						...
						<div class="block-navigation<?php if(get_post_type() == 'thegem_pf_item' && $thegem_pf_data['fullwidth']) { echo ' centered-box'; } ?>">
							<?php if($thegem_nav_post = get_previous_post(false, '', 'thegem_portfolios')) : ?>
								<?php thegem_button(array(
									'text' => __('Projet précédent', 'thegem'),
