<?php
/*
  wp_register_script( 'realocation', get_stylesheet_directory_uri() . '/assets/js/realocation.js' , array( 'owl.carousel' ), false, true );
  wp_enqueue_script( 'realocation' );
*/
?>

<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
  <div class="breadcrumbs single_prop" typeof="BreadcrumbList" vocab="https://schema.org/">
...
<div class="entry-content">
  ...
  <?php $gallery = get_post_meta( get_the_ID(), REALIA_PROPERTY_PREFIX . 'gallery', true ); ?>
    ...
    <div class="hm-property-gallery row">
      <?php $index = 0; ?>
      <?php foreach ( $gallery as $id => $src ) : ?>
          <?php /* $img = wp_get_attachment_image_src( $id, 'large' ); ?>
          <?php $src = $img[0]; */ ?>
          <div class="hm-property-gallery-item col-md-4 col-xs-6">
              ...
              <span class="item-image" style="background-image: url(<?php echo esc_url( $src ); ?>)"></span><!-- /.item-image -->
            </a>
	        </div>
      <?php endforeach; ?>
      
      <?php /* echo do_shortcode('[gallery type="slideshow" link="file" size="medium" ids="45564,45560,45556"]'); */ ?>
...
*/ ?>
    ...
    <div class="tabs">
        ...
        <label for="tab-1"><?=_e('Description', 'tabs' )?></label>
        ...
        <label for="tab-5"><?=_e('A proximité')?></label>
         ...
         <input id="tab-6" type="radio" name="tab-group-1">
        <label for="tab-6" class="green" style="background: #93C01F !important; color: #fff !important;"><?=_e('Demande d’informations')?></label>
        <div class="clear"></div>
        ...
        <div id="contentT6" class="contentT">
     
            <h1><?php echo __('Demande d\'informations', 'own'); ?></h1>
           <div id="enquire_tabs"> <?php echo do_shortcode( '[widget widget_name="Realia_Widget_Enquire"]' ); ?></div>
    ...
    <?php if ( is_array( $subproperties ) && ! empty( $subproperties ) ) : ?>
        <div class="subproperties">
            ...
            if($parentId):
             ...
             ?>
                <a class="btn gem-button btn-brand" style="border-radius: 3px;margin-bottom: 20px;" onmouseleave="" onmouseenter="" href="<?=get_permalink($parentId)?>" target="_self"><?=$parent_post->post_title?></a>
            <?php endif; ?>
            <div class="row">
