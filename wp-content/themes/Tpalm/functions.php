...
function thegem_google_fonts_url() {
	...
	if(count($fonts) > 0) {
		...
		$fonts_url = add_query_arg( $query_args, '//fonts.googleapis.com/css/2' );
		...
		<?php foreach($socials_icons as $name => $active) : ?>
			<?php if($active) : ?>
                <?php if($name==='facebook'):
                    $lienTrad = 'lien-facebook-tpalm'; ?>
                    <a class="socials-item" href="<?=__($lienTrad) ?>" target="_blank" title="<?php echo esc_attr($name); ?>"><i class="socials-item-icon-fa <?php echo esc_attr($name); ?> <?php echo ($type ? 'social-item-'.$type : ''); ?>"></i></a>
                <?php else: ?>
                    <a class="socials-item" href="<?php echo esc_url(thegem_get_option($name . '_link')); ?>" target="_blank" title="<?php echo esc_attr($name); ?>"><i class="socials-item-icon-fa <?php echo esc_attr($name); ?> <?php echo ($type ? 'social-item-'.$type : ''); ?>"></i></a>
                <?php endif; ?>
			<?php endif; ?>
		<?php endforeach; ?>
...
function thegem_get_logo_img($url, $width, $class = '', $echo = 1) {
	$logo = '<img src="'.esc_url(thegem_get_logo_url($url, $width, 1)).'" srcset="'.esc_url(thegem_get_logo_url($url, $width, 1)).' 1x,'.esc_url(thegem_get_logo_url($url, $width, 2)).' 2x,'.esc_url(thegem_get_logo_url($url, $width, 3)).' 3x" alt="'.__('T.PALM, entreprise de construction').'" style="width:'.esc_attr($width).'px;"'.($class ? ' class="'.esc_attr($class).'"' : '').'/>';
...
add_action('init', function () {
     add_rewrite_rule('tpalm-immo/?$','index.php?pagename=tpalm-immo', 'top');
     flush_rewrite_rules();
}, 1000);
